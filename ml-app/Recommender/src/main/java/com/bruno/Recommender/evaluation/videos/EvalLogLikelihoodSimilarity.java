...
public class EvalLogLikelihoodSimilarity {
	public static void main(String... args) throws IOException, TasteException, OptionException {
		...
		IRStatistics evaluation = evaluator.evaluate(
				new LogLikelihoodRecommenderBuilder(), null, model, null, 5, 
				Double.NEGATIVE_INFINITY, 1.0);
		System.out.println(String.valueOf(evaluation));
		
		evaluation = evaluator.evaluate(
				new LogLikelihoodRecommenderBuilder(), null, model, null, 10, 
				Double.NEGATIVE_INFINITY, 1.0);
		System.out.println(String.valueOf(evaluation));
		
		evaluation = evaluator.evaluate(
				new LogLikelihoodRecommenderBuilder(), null, model, null, 20, 
				Double.NEGATIVE_INFINITY, 1.0);
		System.out.println(String.valueOf(evaluation));
	}
}
